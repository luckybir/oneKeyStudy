...
var aCount = 14; //文章默认学习篇数
var vCount = 8; //小视频默认学习个数
var cCount = 2; //收藏+分享+评论次数

var aTime = 60; //每篇文章学习-60秒
      ...
      <viewpager id="viewpager">
        <frame>
          <ScrollView>
            <vertical>
              <button
                text="先点我开启无障碍和悬浮窗,如果没有反应，说明已经开启"
...
ui.click_me.on("click", () => {
  ...
  engines.execScript(
    "选择'自动学习强国'开启无障碍",
    "auto.waitFor();console.show();console.hide();"
  );
});
...
function main() {
  ...
  if (myScores["视听学习"] != 6) {
    videoStudy_bailing(vCount, vTime); //看bailing视频
  }
...
function start_app() {
  ...
  if (className("android.view.View").text("继续挑战").exists()) {
    //双人对战 争上游结束页
    back();
    delay(1);
  }
  ...
  if (className("android.view.View").text("答题练习").exists()) {
    //我要答题页
    back();
    delay(1);
  }
  if (id("menu_contact").exists()) {
    //强国通讯录
    id("home_bottom_tab_button_work").findOne().click();
    delay(1);
  }
  if (id("more_text").exists()) {
    //学习积分页
    back();
    delay(1);
  }
  ...
  /*while (!id("home_bottom_tab_button_work").exists()){
    ...
    !id("home_bottom_tab_button_work").exists()
  ) {
    //20201001 学习按钮文字属性由"学习"改为 "工作"，以下所有点击学习按钮加载主页均同步修改
...
function articleStudy1() {
  ...
  for (var i = 0, t = 0; i < aCount; ) {
    if (click(s, t) == true) {
      //如果点击成功则进入文章页面,不成功意味着本页已经到底,要翻页
      let n = 0;
      ...
      if (
        textContains("央视网").exists() ||
        textContains("广播").exists() ||
        textContains("中央广播电视总台").exists() ||
        textContains("播放").exists() ||
        textContains("展开").exists()
      ) {
        //如果存在“央视网、中央广播电视总台、播放、展开”则认为进入了视频需退出。关键词测试
      ...
      if (id("v_play").exists() || id("bg_play").exists()) {
        //进入电台页面2020.09.28
      ...
      // 点播报
      delay(2);
      var bobao = className("RelativeLayout").depth(11).findOnce(0);
      if (bobao != null) {
        if (bobao.click()) {
          console.log("点击播报");
          i++;
...
function articleStudy2() {
  ...
  for (var i = 0, t = 0; i < aCount; ) {
    ...
    if (art_obj != null && art_obj.parent().childCount() == 4) {
      ...
      if (
        art_obj.parent().child(3).text() == "播报" &&
        art_obj.parent().child(0).text() != currentNewsTitle
      ) {
        //如果播报存在就进入文章正文
...
function stopRadio() {
  ...
  while (
    !(
      textContains("正在收听").exists() ||
      textContains("最近收听").exists() ||
      textContains("推荐收听").exists()
    )
  ) {
    log("等待加载");
    delay(2);
...
function CollectAndShare(i) {
  ...
  var collectIcon = className("ImageView")
    .filter(function (iv) {
...
function zsyQuestion() {
  ...
  delay(3);
  if (
    className("android.widget.Button").text("知道了").exists() ||
    className("android.view.View").text("温馨提示").exists() ||
    className("android.view.View")
      .text("您已超过今日对战次数，请明日再来。")
      .exists()
  ) {
  ...
  while (true) {
    if (
      className("android.view.View").text("继续挑战").exists() ||
      textContains("继续挑战").exists()
    ) {
      //遇到继续挑战，则本局结束
    ...
    if (
      /*textContains("距离答题结束").exists() &&*/ !text("继续挑战").exists()
    ) {
      //20201225答题界面变化 距离答题结束 删除
      zsyQuestionLoop();
    }
  }
}
...
function zsyQuestionLoop() {
  let ClickAnswer;
  try {
    //20201025使用try catch(e)语句处理错误，去除前后置0.5s延时
    /*delay(0.5);*/ //4-0.5，前置0.5延时判断结束标志
    if (
      !className("RadioButton").exists() ||
      className("android.view.View").text("继续挑战").exists() ||
      textContains(
        "继续挑战"
      ).exists() /*|| !textContains("距离答题结束").exists()*/
    ) {
      //不存在本局结束标志 继续挑战，则执行  20201225界面变化，距离答题结束 删除
      ...
      if (className("RadioButton").exists() || aquestion.length == 0) {
        /*delay(0.3);*/
        var aquestion = className("ListView")
          .findOnce()
          .parent()
          .child(0)
          .text();
      ...
      if (aquestion != oldaquestion) {
        if (
          question == ZiXingTi.replace(/\s/g, "") ||
          question == DuYinTi.replace(/\s/g, "") ||
          question == ErShiSiShi.replace(/\s/g, "")
        ) {
        ...
        let listArray = className("ListView").findOnce().children(); //题目选项列表 //如果没找到答案
        /* if (answer == "")*/ if (answer.length == 0) {
          let i = random(0, listArray.length - 1);
          ...
          //如果找到了答案 该部分问题: 选项带A.B.C.D.，题库返回答案不带，char返回答案带
  ...
  while (true) {
    ...
    if (
      ...
      if (lNum >= lCount && conNum >= qCount) {
        ...
        textContains("分享就能复活").exists() ||
        textContains("每周仅可复活一次").exists()
      ) {
        console.log("分享复活...");
        delay(1);
        click("分享就能复活");
...
function weeklyQuestion() {
  ...
  while (true) {
    ...
    while (
      !(
        textStartsWith("填空题").exists() ||
        textStartsWith("多选题").exists() ||
        textStartsWith("单选题").exists()
      )
    ) {
  ...
  questionCollections.children().forEach((item) => {
    if (item.className() != "android.widget.EditText") {
      if (item.text() != "") {
        ...
        /*blankCount += 1;*/
        blankCount =
          className("EditText").findOnce(i).parent().childCount() - 1;
        i++;
      }
    }
  });
...
function checkAndUpdate(question, ansTiku, answer) {
  if (
    className("Button").desc("下一题").exists() ||
    className("Button").desc("完成").exists()
  ) {
    //答错了
    ...
    while (nCout < 5) {
      if (descStartsWith("正确答案").exists()) {
        ...
        var clickPos = className("android.webkit.WebView")
          .findOnce()
          .child(2)
          .child(0)
          .child(1)
          .bounds();
        click(
          clickPos.left + device.width * 0.13,
          clickPos.top + device.height * 0.1
        );
    ...
    if (ansTiku == "" && answer != "") {
      //正确进入下一题，且题库答案为空
      var sql =
        "INSERT INTO tiku VALUES ('" + question + "','" + answer + "','')";
      insertOrUpdate(sql);
      console.log("更新题库答案...");
    }
  }
...
function cUpdateTiku(question, answer, ansTiku) {
  if (ansTiku == "") {
    //题库为空则插入正确答案
    var sql =
      "INSERT INTO tiku VALUES ('" + question + "','" + correctAns + "','')";
  } else {
    //更新题库答案
    var sql =
      "UPDATE tiku SET answer='" +
...
function updateTikunet() {
  ...
  var htmlString = Jsoup.connect("https://qg.zyqq.top/tiku/")
    .maxBodySize(0)
    .timeout(10000)
    .get();
